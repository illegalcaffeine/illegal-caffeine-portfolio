...
function HomepageEditor({ value, projects, patch, upload }: { value: SiteContent["homepage"]; projects: ProjectOption[]; patch: (value: SiteContent["homepage"]) => void; upload: UploadFn }) {
  ...
  return <div className="space-y-10">
    ...
    <SectionTitle title="FEATURED VIDEO" note="요금 절약을 위해 동영상 파일을 Supabase/Vercel에 올리지 않고 YouTube 영상을 임베드해. YouTube URL만 입력하면 돼." />
    <label className="flex items-center gap-3 border border-border p-4 text-sm"><input type="checkbox" checked={value.featured_video.enabled} onChange={(e) => p("featured_video", { ...value.featured_video, enabled: e.target.checked })} className="h-4 w-4" /><span>SHOW VIDEO — 메인 화면에 영상 표시</span></label>
    <TextField label="YOUTUBE URL" value={value.featured_video.youtube_url} onChange={(youtube_url) => p("featured_video", { ...value.featured_video, youtube_url })} />
    <p className="-mt-7 text-xs leading-6 text-muted-foreground">일반 YouTube / youtu.be / Shorts 주소를 그대로 붙여넣으면 돼. 영상 트래픽은 YouTube가 처리해서 Supabase Storage와 Vercel 대역폭을 거의 사용하지 않아.</p>
    <BiField label="VIDEO TITLE" value={value.featured_video.title} onChange={(title) => p("featured_video", { ...value.featured_video, title })} />
    <BiArea label="VIDEO DESCRIPTION" value={value.featured_video.body} onChange={(body) => p("featured_video", { ...value.featured_video, body })} />
