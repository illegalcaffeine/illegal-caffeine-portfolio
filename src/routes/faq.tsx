  ...
  head: () => ({
    meta: [
      { title: "FAQ — Illegal Caffeine - Designer -" },
      {
        name: "description",
        content:
          "Frequently asked questions about Minecraft commissions, delivery, payment, revisions and privacy.",
      },
    ],
  }),
  component: FaqPage,
...
const faq = [
  ...
  },
  {
    ...
    answerEn:
      "Payment terms, milestones and the revision scope are agreed before the build starts. The payment page explains the standard arrangement in more detail.",
    answerKo:
      "결제 조건, 진행 단계와 수정 범위는 작업 시작 전에 합의합니다. 결제 안내 페이지에서 기본 진행 방식을 더 자세히 확인할 수 있습니다.",
  },
  {
      ...
      <section className="border-b border-border px-5 pt-32 pb-12 md:px-10 md:pt-48 md:pb-16">
        <div className="mx-auto max-w-[1600px]">
          ...
          <p className="body-lg mt-8 max-w-2xl">
            {lang === "ko"
              ? "의뢰 전에 자주 확인하는 내용을 정리했습니다. 아래에 없는 내용은 CONTACT에서 직접 문의해 주세요."
              : "Common questions to review before starting a commission. If something is not covered here, contact me directly."}
          </p>
        </div>
      </section>
        ...
        <div className="grid gap-12 md:grid-cols-12 md:gap-16">
          <Reveal className="md:col-span-4">
            ...
            <p className="mt-4 max-w-sm text-sm leading-relaxed text-muted-foreground">
              {lang === "ko"
                ? "프로젝트 범위와 조건은 작업마다 다를 수 있으며, 실제 의뢰 조건은 시작 전에 개별적으로 확정합니다."
                : "Scope and terms can vary by project. Final commission conditions are confirmed individually before work begins."}
            </p>
          </Reveal>
          ...
          <div className="md:col-span-8">
            {faq.map((item, index) => (
              <Reveal
                key={item.en}
                className="grid gap-4 border-t border-border py-8 md:grid-cols-[64px_1fr] md:gap-6 md:py-10"
              >
            ...
            <div className="flex flex-wrap gap-3 border-t border-border pt-8">
              ...
              >
                {lang === "ko" ? "결제 안내" : "PAYMENT INFORMATION"}
              </Link>
            </div>
          </div>
        </div>
