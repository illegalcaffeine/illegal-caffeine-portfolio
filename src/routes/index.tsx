...
const cycle = [
  ...
  { title: "Concept / Blockout", body: "Massing, silhouettes and circulation." },
  { title: "Build", body: "Palette locked, structure detailed, the world built out region by region." },
  { title: "Detail & Revision", body: "Planting, lighting, interiors and the small passes that make it feel inhabited." },
  { title: "Delivery", body: "Schematics or world files handed over." },
];
...
function ProfileBlock({ compact = false }: { compact?: boolean }) {
  return (
    <div className="flex flex-col items-center text-center">
      <div
        className={cn(
          "shrink-0 overflow-hidden rounded-full border border-border-strong bg-surface",
          compact ? "h-32 w-32 sm:h-36 sm:w-36" : "h-24 w-24 md:h-28 md:w-28",
        )}
      >
        <img
          src={profileImage}
          alt="Illegal Caffeine — builder portrait"
          className="h-full w-full origin-center scale-[1.6] object-cover [image-rendering:pixelated] [object-position:center]"
          width={320}
          height={320}
        />
      </div>
